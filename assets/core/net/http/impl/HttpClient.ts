...
export class HttpClient extends Singleton<HttpClient> {
     ...
     * @returns 
     * @example
     M.net.http.get(HttpServer.ApiServer, "/getData", (response) => {
         if (response.isSucc) {
             H.log.info("数据:", response.data);
         } else {
              H.log.error("错误:", response.err);
         }
     }, { id: "12345" });
     */
     ...
     * @example
     var response = await M.net.http.getAsync(HttpServer.ApiServer, "/getData", { id: "12345" });
     */
     ...
     * @returns 
     * @example
     M.net.http.post(HttpServer.ApiServer, "/postData", { name: "test", value: 42 }, (response) => {
         if (response.isSucc) {
             H.log.info("响应数据:", response.data);
         } else {
             H.log.error("错误:", response.err);
         }
     }
