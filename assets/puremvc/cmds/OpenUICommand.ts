...
export default class OpenUICommand extends SimpleCommand {
    ...
    public async execute(notification: INotification): Promise<void> {
        ...
        let uiNode = await M.ui.open(viewId, { viewId, data, preload });
        ...
        if (mediatorCtor && uiNode) {
            if(this.facade.hasMediator(mediatorCtor.name)) {
                H.log.error(`Mediator ${mediatorCtor.name} 已经注册，可能存在重复打开同一界面的问题。`);
                return;
            }
            const mediator = new mediatorCtor(mediatorCtor.name, uiNode, data, viewId);
