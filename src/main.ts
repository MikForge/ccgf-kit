...
interface SceneNode {
    ...
    _name: string;
    _objFlags: number;
    __editorExtras__: Record<string, unknown>;
...
/** 构建节点基础字段 */
function baseNode(name: string, id: string): Partial<SceneNode> {
    return {
        _name: name,
        _objFlags: 0,
        __editorExtras__: {},
 ...
 * @returns 场景 JSON 字符串
 */
export function buildSceneJson(sceneName: string): string {
    // ======== 索引映射 ========
    // 0: SceneAsset
    // 1: Scene
    // 2: root Node
...
/** 场景生成成功后的操作指引文本 */
export const BOOTSTRAP_PROMPT_TEXT = [
    '场景已创建。请手动完成以下步骤：',
    '',
 ...
 * 因此使用固定默认路径而非弹出保存对话框。
 */
async function createBootstrapScene(): Promise<void> {
    try {
        // 1. 自动生成唯一文件名
        ...
        // 3. 生成场景 JSON
        const sceneJson = buildSceneJson(sceneName);
 ...
 * @returns 不冲突的路径
 */
async function resolveScenePath(basePath: string): Promise<string> {
    // 确保以 .scene 结尾
    let target = basePath;
